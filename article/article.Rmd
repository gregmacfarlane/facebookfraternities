...
```{r load_user, warning=FALSE}
source("../rscripts/datacleaner.R")
```
...
```{r distance_from_atlanta}
cities <- read.csv("data/PRIVATEDATA/geocode/geocoded_datatables/allcities.csv")%>%
  tbl_df()
...
```{r save_as_matrices}
# make graph
user_graph <- graph.data.frame(friend_connections)

# export graph edges as adjacency matrix
matrix_list <- list()
for(i in names(friend_connections)[-(1:2)]){
  matrix_list[[i]] <- get.adjacency(user_graph, type = "both", i)
