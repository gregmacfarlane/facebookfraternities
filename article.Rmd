...
library(dplyr, warn.conflicts = FALSE) # super data wrangling
library(igraph) # graph analysis
library(networkD3) # interactive graph visualizer
```
...
-----------------------------------------
The social network data we retrieved from Facebook allow us to construct four social weights matrices, which we describe in this section. To aid this presentation, we must first establish some basic notation following elementary set theory. The set of friends of an individual $i$ is defined as $\phi_i$. The cardinality of this set is the number of friends, so therefore the quantity $|\phi_i \cup \phi_j|$ represents the number of friends that individuals $i$ and $j$ have in common. 
